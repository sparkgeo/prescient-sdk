...
class IngestSpec:
    ...
    def with_uploaded_sources(
        ...
        *,
        ...
        Requires OAuth/IDP auth: the upload uses the STS
        ``assume_role_with_web_identity`` session and is **not supported in
        API-key mode** (accessing the upload session raises there).
